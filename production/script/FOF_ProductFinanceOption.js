$(document).ready(function(){
	...
	if (nowOnlyPrice > 300){
	    ...
	    if (orderTotal >= 2000){
		var financeOption = parentElement.find('#FinanceType [value="ONIF36"]').val();		    
	    } else {
		var financeOption = parentElement.find('#FinanceType').val();
	    }			
	    var financeDeposit =  parentElement.find('#FinanceDeposit').val();									
	    var resetOptions = parentElement.find('#FinanceType').html();

	    try {			   
		var financeCalculator = new FinanceCalculator(financeOption, orderTotal, financeDeposit, rateType, resetOptions); 
		...
		try {
		    var financeCalculator = new FinanceCalculator(financeOption, orderTotal, financeDeposit, rateType); 
		    var financialDetails = financeCalculator.returnFinanceOption();
